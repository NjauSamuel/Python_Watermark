...
def add_watermark(directory, logo_path, position, new_directory, padding, scale):
    ...
    python watermark.py './images' 'logo.png' --pos center --new_dir './watermarked_images'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A script to add watermarks to images. Given a directory, this will traverse through all its "
                    "images and apply the specified watermark. The resulting watermarked images can be saved in the "
                    "same directory or a new specified directory, maintaining the original directory structure.")

    parser.add_argument('dir',
                        help="Directory containing the images you want to watermark. The script will search "
                             "recursively within this directory.",
    ...
    parser.add_argument('--new_dir',
                        default=None,
                        help="An optional directory where the watermarked images will be saved. If not provided, "
                             "watermarked images will overwrite originals in the source directory. The original "
                             "directory structure will be maintained.",
                        metavar='DestinationDirectory')

    parser.add_argument('--padding',
                        type=int,
                        default=0,
                        help="Specifies the padding (in pixels) around the watermark, useful when watermark is "
                             "positioned at the corners. Default is 0, meaning no padding.")
    parser.add_argument('--scale',
                        type=float,
                        default=20,
                        help="Resize the watermark based on a percentage of the image's width. E.g., for 10% of the "
                             "image's width, provide 10.")
